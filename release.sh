...
then
    ...
    docker build . --platform linux/x86_64 -t ghcr.io/milanmdev/bsky.rss:latest #$(git rev-parse --abbrev-ref HEAD)-$(git rev-parse --short HEAD)

    # Push the latest image
    echo "Pushing the image..."
    docker push ghcr.io/milanmdev/bsky.rss:latest #$(git rev-parse --abbrev-ref HEAD)-$(git rev-parse --short HEAD)
else
